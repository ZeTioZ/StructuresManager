...
public class AddMaterialToWhitelist
{
	...
	public boolean command(CommandSender sender, String[] args, String label)
	{
		if(sender.hasPermission("structuresmanager.whitelist.add.material"))
		{
			...
			{
				sendMessage(sender, messages.getStringList("errors.material-already-whitelisted"), prefix, "{label}", label, "{material}", args[3]);
				return true;
			}

			structure.getBlocksWhiteList().add(material);
			sendMessage(sender, messages.getStringList("material-add-success"), prefix, "{label}", label, "{material}", args[3], "{struct_name}", args[2]);
		}
		else
		{
